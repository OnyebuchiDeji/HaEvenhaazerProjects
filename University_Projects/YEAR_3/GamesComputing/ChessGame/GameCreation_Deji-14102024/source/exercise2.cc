...
Pawn::Pawn(PlayerColour colour)
{
    this->colour = colour;
};

Castle::Castle(PlayerColour colour)
{
    this->colour = colour;
};

Knight::Knight(PlayerColour colour)
{
    this->colour = colour;
};

Bishop::Bishop(PlayerColour colour)
{
    this->colour = colour;
};

Queen::Queen(PlayerColour colour)
{
    this->colour = colour;
}

King::King(PlayerColour colour)
{
    this->colour = colour;
};
...
void Player::arrangePieces()
{
    int r, c; for (r = starting_row; r < starting_row + 2; r++) for (c = 0; c < 8; c++) 
    {
        ...
        board[r][c] = new Pawn(player_colour);
        ...
        if (r != pawn_row) {
            ...
            {
            case 3:
                board[r][c] = new King(player_colour);
                break;
            case 4:
                board[r][c] = new Queen(player_colour);
                break;
            case 0:
            case 7:
                board[r][c] = new Castle(player_colour);
                break;
            case 1:
            case 6:
                board[r][c] = new Knight(player_colour);
                break;
            case 2:
            case 5:
                board[r][c] = new Bishop(player_colour);
                break;
            }
        }
    }
...
void Player::carryOutMove()
{
    ...
    cout << "from piece: colour = " << ptr_fromPiece->colour << " type = " << ptr_fromPiece->getType() << "\n";

    //  Initialize the to Piece area
    //  determine the kind of Piece object
    switch (ptr_fromPiece->getType())
    {
    case 'C': ptr_toPiece = new Castle(ptr_fromPiece->colour); break;
    case 'N': ptr_toPiece = new Knight(ptr_fromPiece->colour); break;
    case 'B': ptr_toPiece = new Bishop(ptr_fromPiece->colour); break;
    case 'K': ptr_toPiece = new King(ptr_fromPiece->colour); break;
    case 'Q': ptr_toPiece = new Queen(ptr_fromPiece->colour); break;
    case 'P': ptr_toPiece = new Pawn(ptr_fromPiece->colour); break;
    }

    cout << "to piece: colour = " << ptr_toPiece->colour << " type = " << ptr_toPiece->getType() << "\n";
...
{ // Note: The function that calls validMove should have already checked that the four
  ...
  //       It would be to late to do that here, as this piece would not exist.
    ...
    if (getType() != 'N')                         // Only a knight can jump over pieces.
    {
    ...
    switch (getType())
    {
    case 'P': if ((colour == WHITE) && (dy <= 0)) break; // Check going in the
        ...
        break;
    case 'C': if ((!adx) || (!ady))              return 1; // a valid move
        break;
    case 'N': if ((adx == 2) && (ady == 1))          return 1; // a valid move
        if ((adx == 1) && (ady == 2))          return 1; // a valid move
        break;
    case 'B': if (adx == ady)                    return 1; // a valid move
        break;
    case 'Q':                                    return 1; // a valid move
        // break; // not needed
    case 'K': if ((adx <= 1) && (ady <= 1))          return 1; // a valid move
        break;
