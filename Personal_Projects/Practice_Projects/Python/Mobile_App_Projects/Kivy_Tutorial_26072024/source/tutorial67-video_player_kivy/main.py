"""
    ...
    This episode demonstrates how to add a Video Player.

    Requirements:
        1.  `pip install pillow`
        2.  `pip install ffpyplayer`
        They were not used here, but were probably required by Kivy
"""
...
class MyApp(MDApp):
    ...
    def oninit(self):
        ...
        self.theme_cls.accent_palette = "Thistle"

    def init_video_player(self):
        #   Create the VideoPlayer instance
        player = VideoPlayer(source="resources/DINO_STORM_Medium.mp4")

        #   Assign the VideoPlayer state
        player.state = "play"

        #   'EOS' means 'End Of Stream'
        ##  By default, at the End of Stream, the video stops
        ##  But to change this do:
        player.options = {'eos': 'loop'}    ##  Can bbe 'stop'
        #   To make vidoe width stretch to screen width
        player.allow_stretch = True

        self.root.ids['id_screen'].add_widget(player)
