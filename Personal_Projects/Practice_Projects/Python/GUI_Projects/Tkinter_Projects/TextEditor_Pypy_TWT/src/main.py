"""
    ...
    12. Added keyboard shprtcuts for the open and save file commands.

    A.  I changed what the window title displays, using `os` module so that it
        displays the file name and the directory its gotten from.
        Used os.path.basename to get just the file's name, by getting the end directory item
        Then used os.path.dirname to get the absolute directory of that directory.
...
def open_file(window: tk.Tk, text_edit: tk.Text, x=None):
    print(x)
    ...
    #   The file path without its file name
    pure_filepath = os.path.dirname(filepath)

    #   Change window title
    window.title(f"Open File: {filename}\t | at path: {pure_filepath}")
...
def save_file(window: tk.Tk, text_edit: tk.Text):
    ...
    #   Change window title
    window.title(f"Saving File {filename}\t| to path: {pure_filepath}")
...
def main():
    ...
    #   This then resizes the window itself, because a grid layout is being
    #   used. However if it wasn't a grid layout, one will need to resize it.
    ...
    ##  Adding shortcuts keys
    #   Note to add one parameter, x, because one argument is passed
    #   In the `open_file`, I pass in x to see what it is...
    #   it gives data about the key that was pressed and stuff.
    window.bind("<Control-s>", lambda x: save_file(window, text_edit))
    window.bind("<Control-o>", lambda x: open_file(window, text_edit, x))
