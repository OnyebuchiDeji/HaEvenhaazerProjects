...
""""
        ...
        The changes are clear. But the Video Converter is not implemented here, but in the file...
        PixelArt_VideoCapture.py

        Learnt form CoderSpace Youtube
"""

class ArtConverter:
    def __init__(self, path="Resources/car.jpg", pixel_size=7, color_lvl=8):
        pg.init()
        self.path = path
    ...
    def draw_converted_image(self):
        ...
        for x in range(0, self.WIDTH, self.PIXEL_SIZE):
            for y in range(0, self.HEIGHT, self.PIXEL_SIZE):
                ##  Get desired color
                color_key = tuple(image_color_indices[x, y])
                if sum(color_key):
